...
def get_affine_parameters(theta,pixelsize,mag, fps, v, F_indexmismatch, polarity, dim_height, dim_flow):
    scale_y = (F_indexmismatch * np.cos(theta)); #second dimension
    scale_z = (v / fps / (pixelsize / mag)); #third dimension\
    #if polarity == 1:
    #   theta = -theta
    M_yzscale = np.array([[scale_z, 0, 0],
                [0, scale_y, 0],
    ...
    dim_height_recon = int(np.ceil(dim_height*scale_y))
    dim_flow_recon = int(np.ceil(dim_flow*scale_z + dim_height*scale_y*np.tan(np.abs(theta))))
...
def affine_transform_3D(img_out, img_stack, T, dim_width, dim_flow_recon, dim_height_recon, color, printcurrentloop):
    for i in np.arange(dim_width):
        ...
        #img_out[:,i,:] = cv2.warpAffine(img_stack[:,i,:], T, (dim_flow_recon, dim_height_recon), flags=cv2.INTER_NEAREST)
        if printcurrentloop:
            print('reconstructing' + color + ':' + str(i) + '/' + str(dim_width))

def bg_ex_calib(img_stack, img_bg, img_ex, dim_flow, color, printcurrentloop):
    for i in np.arange(dim_flow):
        img_stack[:,:,i] = (img_stack[:,:,i] - img_bg) / img_ex
        if printcurrentloop:
            print('reading' + color + ':' + str(i) + '/' + str(dim_flow))